...
%% ALGORITHMS
APG                     = 0;            %Alternating Projections based Gridless
ADMM                    = 1;          %Alternating Directions Method of Multipliers
SBL                     = 1;              %Sparse Bayesian Learning
LASSO                   = 0;           %Least Absolute Shrinkage and Selector Operator
ROOTMUSIC               = 0;       %root-MUSIC
IRREGULAR_RM            = 0;     %Irregular root-MUSIC

%% INITIALIZATIONS
T_APGs     = zeros(trials,1);    T_RMs      = zeros(trials,1);
T_iRMs     = zeros(trials,1);     T_ADMMs    = zeros(trials,1);
RMSE_iAPGs = zeros(trials,1); RMSE_RMs   = zeros(trials,1);
RMSE_iRMs  = zeros(trials,1);  RMSE_ADMMs = zeros(trials,1);
RMSE_SBLs  = zeros(trials,1);   Erec_iAPGs  = zeros(trials,1);
...
if trials > 1
    ...
    clf
    if APG
        hold on;        plot(RMSE_iAPGs);        hold off;
    end
    if ADMM
        hold on
        plot(RMSE_ADMMs)
